...
**Acknowledgement (mode 3 only):** an escalation for uncertainty (not the hard list) also sends a fixed, per-language text ("Thanks — I'll check this and get back to you as soon as possible", `core/policy/acknowledge.ts`), never model output, under the same gates as an auto-send (budget ok, per-sender and per-hour caps, no injection signal, Reply-To = From, supported language). The owner gets the escalation with the text quoted; the thread stays escalated, no follow-up is scheduled. At send time it goes out only if the tenant is still in mode 3, else it is cancelled.
...
- **Moved 2026-09-25:** the web app now runs on Cloudflare Workers (OpenNext) at https://app.noctiv.io instead of Netlify; the marketing site is on Cloudflare Pages at https://noctiv.io.
...
## 19. Subscriptions with Paddle Billing (founder decision 2026-09-25)

- **Seller:** Paddle is merchant of record: checkout, VAT/sales tax by buyer country (B2B reverse charge), invoices, payment retries, customer portal. One product "Noctiv", one price $79/month, tax-exclusive. Sandbox until live keys (`PADDLE_ENV=sandbox|production`).
- **Trial:** stays in the app, no card: `tenants.trial_ends_at` (created_at + 14 days). The dashboard shows a Plan card with Subscribe at any time; a banner appears 3 days before the end and after it. Subscribe opens Paddle Checkout as an overlay (Paddle.js v2) with `customData.tenant_id` and the owner's email.
- **Trial countdown and reminders (2026-09-25):** every page header shows "Free trial — N days left" for the whole trial (amber in the last 3 days); the Plan card shows the exact end in the business's time zone. The worker's hourly run calls `app.queue_trial_reminders()`, which queues one owner e-mail (`trial_ending`) when 7 days or fewer are left and one in the last day, deduplicated per trial end date; subscribed, comped and deleted businesses get none.
- **Webhook:** `POST /paddle/webhook` on the API (outside `/v1`, no user token; reachable as `https://app.noctiv.io/api/paddle/webhook`). The `Paddle-Signature` HMAC is checked on the raw body (5-minute tolerance). `subscription.*` events call `app.paddle_apply_subscription()` (SECURITY DEFINER, API only): tenant found by subscription id → custom data → customer id; older events for the same subscription are ignored (`stale`); a late cancel of an old subscription never stops a newer one; every event is written to `audit_log` (`billing.<event_type>`). Other event types get 200 and are ignored.
- **Status → service:** `app.billing_entitled(status, trial_ends_at)` is true for `active`, `trialing`, `past_due` (Paddle is retrying; dunning grace), `comped` (operator-set, free) and an unexpired `trial`. When false: the mailbox listener and health checks stop (`app.list_mail_connections`), follow-ups stop (`app.due_followups`), `mail.fetch` and the pipeline skip with `billing_inactive`, and queued automatic sends are downgraded to approval. Owner-approved sends still go out. Data is kept.
- **Reactivation:** a webhook that makes the tenant entitled again sets `billing_resumed_at`; mail that arrived while service was stopped is then read, but only ever drafted for approval (reason `arrived_while_paused`), never answered automatically.
- **Portal:** Settings → Manage billing creates a Paddle customer portal session (`POST /customers/{id}/portal-sessions`) for card changes, invoices and cancellation. A cancellation scheduled for period end is shown ("keeps working until …").
- **Account deletion** cancels an open subscription immediately through the Paddle API.
- **Env (API):** `PADDLE_ENV`, `PADDLE_API_KEY`, `PADDLE_WEBHOOK_SECRET`, `PADDLE_CLIENT_TOKEN`, `PADDLE_PRICE_ID`. The web app gets the client token and price id from `GET /v1/tenants/:id/billing`, so no web rebuild is needed when keys change.
- **Site:** "$79/month, plus VAT where applicable" on the price panel; Terms name Paddle as merchant of record; Privacy and `subprocessors.md` list Paddle; new `/refunds/` page (14-day trial, cancel any time, no refunds for partial months unless the service failed), linked in the footer.
- **Status 2026-09-25:** live keys set on the API (`PADDLE_ENV=production`; no webhook secret yet). Price `pri_01m3ckhnwcpmb9cadya6xpw6kc` read through Paddle.js pricing preview: $79.00 USD monthly, product "Noctiv", but `tax_mode: internal` (tax-inclusive) and a 14-day card-required trial on the price; both must change (tax exclusive, no trial). Live checkout returns `transaction_checkout_not_enabled` until Paddle onboarding (identity, business, domain review) is complete. The live API key has no price, product or transaction read access (not needed by the app).
- **Tests:** `api/test/paddle.test.ts` (signature, parsing, API client), `api/test/billing.db.test.ts` (webhook ordering, entitlement, portal, grants), worker pipeline and send tests for the paused and resumed cases.

## 20. E-mail design per tenant (founder decision 2026-09-26)

- **What:** the visual frame around every outgoing reply, follow-up and acknowledgement, in every mode. The reply text is never changed. Settings → E-mail design, with a live preview (light, and dark as mail apps invert it) of a sample reply.
- **Five designs** (`tenants.email_template`): `plain` (default; text only, exactly what was sent before), `clean` (light HTML, 600 px column, divider, text signature, no images), `logo` (clean + logo, max 160 px wide), `branded` (4 px brand-colour bar, logo, signature block with links, footer with company and address), `card` (white card on light grey, logo top left, links as brand-colour buttons).
- **Fields:** company name, logo URL, brand colour, website, phone, address, up to 3 social links (`tenants.brand_*`).
...
- **Numbering:** per type, per tenant, restarting each calendar year in the tenant's time zone, like quotes: `INV-2026-0001`, `DN-2026-0001`, `CMR-2026-0001`. Next number = highest of that type and year + 1, tenant row locked; unique `(tenant_id, number)`. *Decision 2026-09-26:* each business sets its own prefix per type in Settings → Documents (default INV / DN / CMR; 1–10 capital letters or digits; the three must differ). A prefix is fixed for the rest of the year once a document of that type has been issued, so a year's series never mixes prefixes and has no gaps. "Ready" (issued) confirmed as the step that gives the number.
...
- **VAT:** the tenant's mode (none / added / included) and rate, snapshotted on the invoice. **Reverse charge** flag: VAT 0 and the standard note in the invoice language ("Reverse charge: VAT to be accounted for by the recipient, Art. 196 Directive 2006/112/EC"); requires seller and buyer VAT numbers. With VAT-inclusive prices (*decision 2026-09-26*) each unit price is recalculated to net (price × 100 / (100 + rate), half-up to the cent) and the line totals follow from the net prices, so every line still reads qty × unit price = total.
...
- **Created** from an invoice (buyer, lines with qty, units and prices copied) or manually.
- **With prices (pavadzīme-rēķins)**, *decision 2026-09-26*: an option on the note, default off. When on, the lines show unit price and total, the note shows subtotal / VAT / total like an invoice (the tenant's VAT setting) and the bank details, and it is titled "Preču pavadzīme-rēķins" (en "Delivery note and invoice", de "Lieferschein und Rechnung", …). Every line then needs a price, and the seller's VAT number is needed when VAT applies. It is still marked delivered, not paid.
